...
export default function CustomerSearchWithPaidByMonth({
  initialCustomers,
  bills,
  ...
  const paymentStatusBadge = (c: Customer): { label: string; tone: string } => {
    if (c.isMonitor) return { label: "Monitor", tone: "bg-amber-100 text-amber-800" };
    if (c.billingType === "FREE") return { label: "Free", tone: "bg-sky-100 text-sky-800" };

    const bill = billByCustomerAndMonth.get(`${c.customerId}|${monthKey}`);
    if (!bill) return { label: "—", tone: "bg-slate-100 text-slate-600" };

    if (bill.paymentStatus === "PAID") return { label: "PAID", tone: "bg-green-100 text-green-800" };
    if (bill.paymentStatus === "PARTIAL") return { label: "PARTIAL", tone: "bg-amber-100 text-amber-800" };
    return { label: "UNPAID", tone: "bg-slate-100 text-slate-600" };
  };
      ...
      <div className="bg-white rounded-lg border border-slate-200 overflow-x-auto mt-4">
        <table className="min-w-[720px] w-full divide-y divide-slate-200">
          ...
          <tbody className="divide-y divide-slate-200">
            {filtered.map((c) => {
              const badge = paymentStatusBadge(c);
              return (
                <tr key={c.customerId} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-right">
                    <span className={`inline-flex px-2 py-0.5 text-xs font-medium rounded ${badge.tone}`}>
                      {badge.label}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-right">
